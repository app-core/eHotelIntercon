...
public class CoreMedia extends HttpServlet {
	...
	private static VodContentDAO2 voddao = DAOFactory.getInstance()
			.getModContentDAO2();
	...
	private static BrowserProgDao livedao = DAOFactory.getInstance()
			.getBrowserProgDao();
	private static RecordContent recorddao = DAOFactory.getInstance()
			.getRecordContent();
	...
	public CoreMedia() {
		...
		if (config.remoteviewport > 0) {
			System.out.println("Start RemoteViewServer with port "
					+ config.remoteviewport);
			remoteViewServer = new RemoteViewServer(config.remoteviewport,
					"SERVER APP MODULE");
	...
	protected void doProcess(HttpServletRequest request,
		...
		int lenh = new Integer(request.getParameter(Command.command))
				.intValue();
		...
		// " - curent time stb: ");
		System.out.println("Ip: " + ip + " - keystb: " + keystb + " - lenh : "
				+ lenh);
		...
		if (lenh == Command.com_getListRecordStb) {
			...
			System.out.println(lenh + " | " + tenkenh + " | " + starttime
					+ " | " + stoptime + " | " + idkenh);
			recorddao.setScheduleStb(idkenh, tenkenh, keystb, starttime,
					stoptime);
			out.println("OK");
			return;
		...
		/*
		 * if (lenh == Command.com_updateStatusCore){ String idrecord =
		 * request.getParameter(Param.idrecord); String status =
		 * request.getParameter(Param.status);
		 * recorddao.updateStatusCore(idrecord, status); out.println("OK"); }
		 */
		...
		if (lenh == Command.com_getlang) {
			System.out.println("Ip: " + ip + " - In setLang(" + keystb + ","
					+ langid + ")");
			...
			System.out.println("Ip: " + ip + " - In setKeyStbIntoRoom("
					+ keystb + "," + room + ")");
			...
			System.out.println("Ip: " + ip + " - In register(" + keystb + ","
					+ room + "," + ip + ")");
			...
			System.out.println("Ip: " + ip + " - SN: " + keystb + " - idapk: "
					+ idapk);
		...
		if (lenh == Command.com_static) {
			int id_content = new Integer(request.getParameter(Param.contentid))
					.intValue();
			String type = request.getParameter(Param.type);
			System.out.println("Ip: " + ip + " - In statics(" + keystb + ","
					+ id_content + "," + type + ")");
		...
		if (lenh == Command.com_chargeVod) {
			 ...
			 * String servicename, String typeprice, String nameguest,String
			 * pincode
			 */
			int contentid = new Integer(request.getParameter(Param.contentid))
					.intValue();
			...
			System.out.println("Ip: " + ip + " - In chargeVod(" + keystb + ","
					+ contentid + "," + price + ")");
			int xml = voddao.chargeVod(keystb, contentid, price, namecontent,
					servicename, typeprice, nameguest, pincode);
			System.out.println(xml);
			out.println(xml);
			return;
		} else
		...
		if (lenh == Command.com_checkpaid) {
			...
			System.out.println("Ip: " + ip + " - In checkpaid(" + keystb + ","
					+ contentid + ")");
		...
		if (lenh == Command.com_getlistsubjectvod) {
			...
			System.out.println("Ip: " + ip + " - In getAllNorSubjects("
					+ keystb + ")");
			String type = request.getParameter(Param.type);
		...
		if (lenh == Command.com_getlistfileofsubject) {
			short subjectid = new Short(request.getParameter(Param.subjectid))
					.shortValue();
			int fromRow = new Integer(request.getParameter(Param.fromRow))
					.intValue();
			int noRows = new Integer(request.getParameter(Param.noRows))
					.intValue();
			System.out.println("Ip: " + ip + " - In getMODCtnIDsOfSubjectNew("
					+ keystb + "," + subjectid + "," + fromRow + "," + noRows
					+ ")");
			String json = voddao.getMODCtnIDsOfSubjectNew(keystb, subjectid,
					fromRow, noRows);
			System.out.println(json);
			out.println(json);
			return;
		...
		if (lenh == Command.com_getountilm) {
			vodsubjectid = new Integer(request.getParameter(Param.subjectid))
					.intValue();
		...
		if (lenh == Command.com_getlisturlsub) {
			System.out.println("Ip: " + ip + " - In getUrlSub("
					+ request.getParameter(Param.contentid) + ")");
			int ctnid = new Integer(request.getParameter(Param.contentid))
					.intValue();
		...
		if (lenh == Command.com_getModListSongSubject) {
			...
			int noRows = new Integer(request.getParameter(Param.noRows))
					.intValue();
			System.out.println("Ip: " + ip + " - In getModListSongSubject("
					+ keystb + "," + subjectid + "," + fromRow + "," + noRows
					+ ")");
			String xml = moddao.getModListSongSubject(keystb, subjectid,
					fromRow, noRows);
			System.out.println(xml);
			out.println(xml);
			return;
		...
		if (lenh == Command.com_getountmusic) {
			int subjectid = new Integer(request.getParameter(Param.subjectid))
					.intValue();
		...
		if (lenh == Command.com_getlivesubject) {
			...
			int noRows = new Integer(request.getParameter(Param.noRows))
					.intValue();
			System.out.println("Ip: " + ip + " - In getLiveTvSubject(" + keystb
					+ "," + fromRow + "," + noRows + ")");
		...
		if (lenh == Command.com_getlivechannel) {
			System.out.println("Ip: " + ip + " - In getLiveTvSubject("
					+ channelid + ")");
			String json = livedao.getLivetvChannelList(new Integer(channelid)
					.intValue());
			System.out.println(json);
			out.println(json);
			return;
		...
		if (lenh == Command.com_getMessageInfo) {
			int messId = new Integer(request.getParameter(Param.messId))
					.intValue();
			...
			int folderid = new Integer(request.getParameter(Param.folderid))
					.intValue();
			...
			System.out.println("lenh getWelcomeMessage: " + lenh
					+ " - keystb: " + keystb);
			...
			String json = pmsdao
					.getItemOfService(subMenuId, keystb, ffrom, tto);
			System.out.println(json);
			out.println(json);
			return;
